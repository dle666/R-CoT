...
def conv2text(sources):
    ...
    for idx, sentence in enumerate(sources):
        ...
        if from_str.lower() == 'human' or from_str.lower() == 'user':
            ...
            # print("_____________________________________________________________", sentence['value'])

            temp = (
                BEGIN_SIGNAL + from_str + str(sentence['value']).strip() + END_BOT)
        conversation += temp
...
class Mix_dataset(Dataset):
    ...
    def __init__(self, json_datas, batch_size=1, img_size=224, local_rank=0):
        ...
        self.data_ratio_multi = [float(ratio) / sum(self.data_num_multi) 
                                for ratio in self.data_num_multi]
        self.data_ratio_text = [float(ratio) / sum(self.data_num_text)
